...
class Ssd:
    ...
    def save_data(self, file):
        var = os.path.getsize(file)
        self.volume = self.volume + var
        return f'\nОбъем данных на диске увеличился на {var} байт и составляет {self.volume} байт.'

    def remove_data(self, file):
        var = os.path.getsize(file)
        self.volume = self.volume - var
        return f'Объем данных на диске уменьшился на {var} байт и составляет {self.volume} байт.'
...
class PowerUnit(Motherboard, Cpu):
    def __init__(self, on=None):
        super().__init__()
        self.power:int = power
        self.on = on
    ...
    def get_power(self):
        if self.on:
            ...
            self.redistribution_energy()
        return f'Питание компьютера выключено'


class MyPc(PowerUnit, Ram, Ssd, VideoCards):
    def __init__(self):
        super().__init__()
        self.memory = memory
        self.volume = volume_ssd
        self.video_card = video_card_name
        self.memory_card = video_card_memory
    ...
    def start_modules(self):
        ...
        print(self.activate_turbo_mode(turbo=True) if foo('\nВы хотите активировать turbo-режим? y/n -> ')
              else self.activate_turbo_mode(turbo=False))

        # оперативная память.
        print(self.load_data())                     # Загрузка данных
        print(self.upload_data())                   # Выгрузка данных

        # SSD накопитель
        print(self.save_data('file.txt'))           # Сохранение данных
        print(self.remove_data('file.txt'))         # Удаление данных

        # Видео карта
        print(self.show_picture())                  # Показ изображения
